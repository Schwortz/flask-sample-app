...
Generated at: 2026-01-14T14:53:34.601568+00:00
...
TEST_CASES = json.loads('''[
    {
        ...
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Get items from an empty list returns empty array",
        "request_data": {
            "body": null,
            "path": {},
            "query": {}
    ...
    {
        "name": "add_item_simple_object",
        "setup": null,
        "method": "POST",
        "cleanup": null,
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Successfully add a simple item with basic fields",
        "request_data": {
            "body": {
                "name": "Test Item",
                "quantity": 5,
    ...
    {
        "name": "add_item_nested_data",
        "setup": null,
        "method": "POST",
        "cleanup": null,
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Successfully add an item with nested objects and arrays",
        "request_data": {
            "body": {
                "tags": [
                    "electronics",
    ...
    {
        "name": "add_item_empty_object",
        "setup": null,
        "method": "POST",
        "cleanup": null,
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Successfully add an empty JSON object as an item",
        "request_data": {
            "body": {},
            "path": {},
            "query": {}
    ...
    },
    {
        ...
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Retrieve all items after adding multiple items",
        "request_data": {
            "body": null,
            "path": {},
            "query": {}
    ...
    {
        "name": "add_item_complex_types",
        "setup": null,
        "method": "POST",
        "cleanup": null,
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Add an item with various JSON data types including null",
        "request_data": {
            "body": {
                "nullField": null,
                "arrayField": [
    ...
    {
        "name": "add_item_array_of_objects",
        "setup": null,
        "method": "POST",
        "cleanup": null,
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Add an item containing an array of complex objects",
        "request_data": {
            "body": {
                "users": [
                    {
    ...
    {
        "name": "add_item_special_characters",
        "setup": null,
        "method": "POST",
        "cleanup": null,
        "category": "HAPPY_PATH",
        "endpoint": "/items",
        "description": "Add an item with special characters and Unicode",
        "request_data": {
            "body": {
                "name": "Special Item",
                "emoji": "\ud83d\ude00\ud83c\udf89\ud83d\ude80",
                "unicode": "Hello \u4e16\u754c \ud83c\udf0d \u041f\u0440\u0438\u0432\u0435\u0442",
                "description": "Contains special chars: @#$%^&*()_+-=[]{}|;':,.<>?/~`"
            },
            "path": {},
            "query": {}
        },
...
BASE_URL = os.path.expandvars("http://localhost:$PORT")
HEALTH_CHECK_ENDPOINT = os.path.expandvars("")
# Full health check URL (same as healthcheck_command uses)
HEALTH_CHECK_URL = f"{BASE_URL.rstrip('/')}{HEALTH_CHECK_ENDPOINT}"
...
def validate_contract(
    ...
    """
    Validate that actual response conforms to the expected contract/schema.

    This is NOT an exact value comparison. Instead, it validates:
    1. Structure: All expected fields exist in actual
    2. Types: Values have compatible types (string, number, array, object, null, boolean)
    3. Arrays: Same length (structure validation)
    4. Strings: Non-empty strings in expected should be non-empty in actual
    5. Dynamic fields: Only checked for presence and type, values can differ

    Args:
        actual: The actual response from DST
        expected: The expected response structure (from SRC's actual_response)
        path: Current path in the response (for error messages)

    Returns:
        tuple: (is_valid, list of contract violations)
    """
    violations: list[str] = []
    ...
    # Object/dict validation - check structure
    if isinstance(expected, dict):
        for key, exp_val in expected.items():
            ...
            if is_dynamic:
                ...
                # For non-dynamic fields, recursively validate contract
                valid, sub_violations = validate_contract(act_val, exp_val, key_path)
                violations.extend(sub_violations)

        return len(violations) == 0, violations

    # Array validation - check length and validate each item's structure
    if isinstance(expected, list):
        # Length validation (contract says we expect N items)
    ...
    # String validation - check non-empty constraint
    if isinstance(expected, str):
        if expected.strip() and not actual.strip():
            violations.append(f"{path or 'root'}: expected non-empty string, got empty string")
            return False, violations
        return True, []

    # Number validation - just type check (already done above)
    if isinstance(expected, (int, float)):
        return True, []

    # Boolean validation - just type check (already done above)
...
def record_result(
    ...
    if response_diff:
        result["response_diff"] = response_diff[:10]  # Limit stored diffs
    ...
    if response_body:
        if passed:
            ...
            try:
                ...
                result["response_body"] = response_body[:2000]
        else:
            result["response_body"] = response_body[:500]
...
def make_request(
    method: str, endpoint: str, path_params: dict[str, Any], query_params: dict[str, Any], body: Any
) -> requests.Response:
    ...
    method = method.upper()
    if method == "GET":
        return requests.get(url, headers=headers, cookies=cookies, timeout=30)
    elif method == "POST":
        return requests.post(url, headers=headers, cookies=cookies, json=body, timeout=30)
    elif method == "PUT":
        return requests.put(url, headers=headers, cookies=cookies, json=body, timeout=30)
    elif method == "PATCH":
        return requests.patch(url, headers=headers, cookies=cookies, json=body, timeout=30)
    elif method == "DELETE":
        return requests.delete(url, headers=headers, cookies=cookies, timeout=30)
    else:
        return requests.request(method, url, headers=headers, cookies=cookies, json=body, timeout=30)
...
def run_cleanup(cleanup_config: dict[str, Any], setup_id: str | None) -> None:
    ...
    try:
        ...
        if resp.status_code >= 400:
            print(f"Cleanup warning: {resp.status_code} - {resp.text[:100]}")
    except Exception as e:
...
def test_api_endpoint(test_case: dict[str, Any]) -> None:
    ...
    try:
        ...
        url = build_request_url(endpoint, path_params, query_params)
        headers = build_headers(req_headers, skip_auth=skip_auth)
        ...
        try:
            if method == "GET":
                ...
                resp = requests.post(url, headers=headers, cookies=cookies, json=body, timeout=30)
            elif method == "PUT":
                resp = requests.put(url, headers=headers, cookies=cookies, json=body, timeout=30)
            elif method == "PATCH":
                resp = requests.patch(url, headers=headers, cookies=cookies, json=body, timeout=30)
            elif method == "DELETE":
                resp = requests.delete(url, headers=headers, cookies=cookies, timeout=30)
            else:
                resp = requests.request(method, url, headers=headers, cookies=cookies, json=body, timeout=30)
